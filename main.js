...
function setStyle() {
  ...
  switch (themeDropdown.value) {
    case "red":
      themeStylesheet.href = "simpleRed.css";
    case "light":
...
function startTimer() {
  ...
  if (isIncrementalTimer()) {
    ...
    pauseButton.addEventListener(
      "click",
      function decrementPauseListener() {
        timerPaused = true;
        timerDecrementTime = ms;
        clearInterval(timer);
      },
...
function manualTimeSubmit() {
  ...
  if (
    inputIsValid(hours) &&
    inputIsValid(minutes) &&
    inputIsValid(seconds) &&
    inputIsValid(ms)
  )
    setDecrementTime(
      3600000 * parseInt(hours.value) +
        60000 * parseInt(minutes.value) +
        1000 * parseInt(seconds.value) +
        parseInt(ms.value),
    );
  else alert("Invalid value(s)!");
}
...
function inputIsValid(inputElement) {
  ...
  return !(
    ...
    elementValue > elementMax ||
    elementValue < elementMin
  );
}
